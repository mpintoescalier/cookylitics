...
const initialHistoryRows = [
  { date: "17 May", action: "Predicción semanal generada", result: "1,340 unidades", confidence: "94.2%", impact: "+7.6% demanda", period: "18 May a 24 May", events: ["Partido importante", "Feriado"], detail: "Se proyectó una semana de alta demanda con refuerzo de pizzas y burgers." },
...
function CalculatorPage({ recipes, stockItems, setStockItems, events, setEvents, addHistory, showToast, openEventModal }) {
  ...
  function calculate() {
    ...
    addHistory({
      ...
      period: `${dateRange.from} a ${dateRange.to}`,
      events: activeEvents.map((event) => event.name),
      detail: `${result.totalProjectedPlates.toLocaleString("en-US")} platos estimados con ${activeEvents.length} evento(s) especial(es).`,
    });
...
function HistoryPage({ historyRows, clearHistory, openHistoryModal }) {
  return <><SectionTitle title="Historial" subtitle="Registro de predicciones, eventos, pedidos y cambios de configuración." action={<button onClick={clearHistory} className="h-12 rounded-2xl border border-rose-400/20 bg-rose-400/10 px-5 font-black text-rose-200 hover:bg-rose-400/15">Limpiar historial</button>} /><Card className="overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-[940px] text-left"><thead className="border-b border-white/[0.07] bg-white/[0.025] text-slate-400"><tr><th className="px-6 py-5">Fecha</th><th className="px-6 py-5">Acción</th><th className="px-6 py-5">Resultado</th><th className="px-6 py-5">Confianza</th><th className="px-6 py-5">Impacto</th><th className="px-6 py-5"></th></tr></thead><tbody>{historyRows.map((row, index) => <tr key={index + row.action} className="border-b border-white/[0.05] hover:bg-white/[0.025]"><td className="px-6 py-5 font-bold text-slate-300">{row.date}</td><td className="px-6 py-5 font-black text-white">{row.action}</td><td className="px-6 py-5 text-slate-300">{row.result}</td><td className="px-6 py-5 text-[#12a8c7]">{row.confidence}</td><td className="px-6 py-5 text-emerald-400">{row.impact}</td><td className="px-6 py-5"><button onClick={() => openHistoryModal(row)} className="rounded-xl bg-[#7c4dff]/15 px-4 py-2 text-sm font-black text-[#b49aff] hover:bg-[#7c4dff]/25">Ver detalle</button></td></tr>)}{!historyRows.length && <tr><td colSpan="6" className="px-6 py-12 text-center text-slate-500">Historial vacío.</td></tr>}</tbody></table></div></Card></>;
}
...
function TextArea({ label, value, onChange, placeholder = "" }) {
  return <label><span className="mb-2 block text-sm font-bold text-slate-400">{label}</span><textarea value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} rows="4" className="w-full resize-none rounded-2xl border border-white/[0.07] bg-[#101118] px-4 py-3 font-bold text-white outline-none placeholder:text-slate-600 focus:border-[#8f63ff]/60" /></label>;
}
...
function RecipeModal({ recipe, mode, onClose, onSave }) {
  ...
  function addIngredient() { setDraft((current) => ({ ...current, ingredients: [...current.ingredients, { name: "", qty: 0, unit: "kg" }] })); }
  function updateIngredient(index, field, value) { setDraft((current) => ({ ...current, ingredients: current.ingredients.map((ingredient, currentIndex) => currentIndex === index ? { ...ingredient, [field]: field === "qty" ? Number(value) : value } : ingredient) })); }
  function removeIngredient(index) { setDraft((current) => ({ ...current, ingredients: current.ingredients.filter((_, currentIndex) => currentIndex !== index) })); }
  return <Modal title={readOnly ? recipe.name : recipe ? `Editar ${recipe.name}` : "Nueva receta"} onClose={onClose} footer={readOnly ? <button onClick={onClose} className="h-12 w-full rounded-2xl bg-[#7c4dff] font-black text-white">Cerrar</button> : <div className="grid grid-cols-2 gap-3"><button onClick={onClose} className="h-12 rounded-2xl border border-white/[0.08] bg-white/[0.035] font-black text-white">Cancelar</button><button onClick={() => onSave(draft)} className="h-12 rounded-2xl bg-[#7c4dff] font-black text-white">Guardar receta</button></div>}>{readOnly ? <div className="space-y-5"><div className="text-6xl">{recipe.emoji}</div><p className="text-slate-400">{recipe.instructions}</p><div className="flex flex-wrap gap-2">{recipe.ingredients.map((ingredient) => <span key={ingredient.name} className="rounded-xl bg-white/[0.055] px-3 py-2 text-sm font-bold text-slate-300">{ingredient.qty} {ingredient.unit} {ingredient.name}</span>)}</div><div className="grid grid-cols-2 gap-3"><MetricMini label="Costo" value={`$${recipe.cost}`} /><MetricMini label="Precio" value={`$${recipe.price}`} /></div></div> : <div className="space-y-5"><div className="grid gap-4 md:grid-cols-2"><TextInput label="Nombre" value={draft.name} onChange={(value) => setDraft({ ...draft, name: value })} /><TextInput label="Categoría" value={draft.category} onChange={(value) => setDraft({ ...draft, category: value })} /><TextInput label="Emoji" value={draft.emoji} onChange={(value) => setDraft({ ...draft, emoji: value })} /><TextInput label="Costo" type="number" value={draft.cost} onChange={(value) => setDraft({ ...draft, cost: Number(value) })} /><TextInput label="Precio" type="number" value={draft.price} onChange={(value) => setDraft({ ...draft, price: Number(value) })} /><TextInput label="Cantidad histórica" type="number" value={draft.baseDemand} onChange={(value) => setDraft({ ...draft, baseDemand: Number(value) })} /></div><TextArea label="Proceso de preparación" value={draft.instructions} onChange={(value) => setDraft({ ...draft, instructions: value })} placeholder="Describí los pasos principales de la receta" /><div><div className="mb-3 flex items-center justify-between"><h4 className="font-black text-white">Ingredientes y cantidades</h4><button onClick={addIngredient} className="rounded-xl bg-[#18d4ef]/15 px-3 py-2 text-sm font-black text-cyan-300">+ Agregar ingrediente</button></div><div className="space-y-3">{draft.ingredients.map((ingredient, index) => <div key={`${ingredient.name}-${index}`} className="grid gap-2 rounded-2xl border border-white/[0.07] bg-white/[0.025] p-3 md:grid-cols-[1fr_100px_100px_auto]"><TextInput label="Ingrediente" value={ingredient.name} onChange={(value) => updateIngredient(index, "name", value)} /><TextInput label="Cantidad" type="number" value={ingredient.qty} onChange={(value) => updateIngredient(index, "qty", value)} /><TextInput label="Unidad" value={ingredient.unit} onChange={(value) => updateIngredient(index, "unit", value)} /><button onClick={() => removeIngredient(index)} className="self-end rounded-xl bg-rose-400/10 px-3 py-3 font-black text-rose-300">Quitar</button></div>)}</div></div></div>}</Modal>;
}

function HistoryDetailModal({ row, onClose }) {
  return <Modal title="Detalle de predicción" onClose={onClose} footer={<button onClick={onClose} className="h-12 w-full rounded-2xl bg-[#7c4dff] font-black text-white">Cerrar detalle</button>}><div className="space-y-4"><div className="rounded-2xl border border-[#7c4dff]/20 bg-[#7c4dff]/10 p-5"><p className="text-sm font-black uppercase tracking-[0.16em] text-[#b49aff]">{row.date}</p><h4 className="mt-2 text-2xl font-black text-white">{row.action}</h4><p className="mt-2 text-slate-400">{row.detail || "Registro operativo guardado por Cookylitics."}</p></div><div className="grid grid-cols-2 gap-3"><MetricMini label="Resultado" value={row.result} /><MetricMini label="Confianza" value={row.confidence} /><MetricMini label="Impacto" value={row.impact} /><MetricMini label="Período" value={row.period || "No informado"} /></div><div><p className="mb-3 font-black text-white">Eventos considerados</p><div className="flex flex-wrap gap-2">{(row.events || ["Sin eventos especiales"]).map((event) => <span key={event} className="rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-2 text-sm font-black text-cyan-200">{event}</span>)}</div></div></div></Modal>;
}
...
export default function CookyliticsApp() {
  ...
  const [currentScreen, setCurrentScreen] = useState(screens.some((screen) => screen.id === requestedScreen) ? requestedScreen : "dashboard");
  ...
  const [modal, setModal] = useState(() => {
    if (urlParams.get("modal") === "new-recipe") return { type: "recipe", recipe: null, mode: "create" };
    if (urlParams.get("modal") === "history-detail") return { type: "history", row: initialHistoryRows[0] };
    return null;
  });
  ...
  const pages = {
    ...
    history: <HistoryPage historyRows={historyRows} clearHistory={clearHistory} openHistoryModal={(row) => setModal({ type: "history", row })} />,
